...
<Note>Requests exceeding this limit return `400 Bad Request`. Split larger collections into multiple batches (for example, up to 50,000 places per batch).</Note>
...
- **Chunk large collections** — Stay under 50K jobs per request; split into multiple batches or append via repeated requests with the same `batch_id`.
