...
- `reason_bucket` (enum): The high-level category explaining why the status was assigned
...
Each status determination falls into one of the following buckets, returned as `open_closed_status_reason_bucket`:

```python
class OpenClosedReasonBucket(str, Enum):
    ACTIVE_DIGITAL_PRESENCE = "active_digital_presence"
    MINIMAL_DIGITAL_PRESENCE = "minimal_digital_presence"
    EXPLICIT_CLOSURE_SIGNAL = "explicit_closure_signal"
    NO_RECENT_DIGITAL_PRESENCE = "no_recent_digital_presence"
    OFFICIAL_TEMPORARY_CLOSURE = "official_temporary_closure"
    NO_INTERNET_PRESENCE = "no_internet_presence"
```

| Bucket | Typical Status | Description |
|--------|---------------|-------------|
| `active_digital_presence` | Open | Official website, active social media, recent aggregator listings — no closure evidence |
| `minimal_digital_presence` | Open | Some web presence (e.g. official website + gov filing) but limited overall — often medium confidence |
| `explicit_closure_signal` | Closed | First-party or corroborated third-party sources explicitly indicate permanent closure |
| `no_recent_digital_presence` | Closed | Customer-facing storefront with no official presence and only outdated third-party listings |
| `official_temporary_closure` | Temporarily Closed | Official first-party or social sources indicate a temporary closure |
| `no_internet_presence` | No Internet Presence | No exact or near-exact name match and no address match tied to the business name on web sources |
...
```json
{
  ...
  "outputs": {
    "closed_permanently": {
      "status": "Open",
      "confidence": "HIGH",
      "reason_bucket": "active_digital_presence",
